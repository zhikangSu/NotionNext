...
export async function resolvePostProps({ prefix, slug, suffix, locale, from }) {
  ...
  const isHiddenDirectPage =
    isHiddenPageId(fullSlug, props.NOTION_CONFIG) ||
    isHiddenPageId(lastSegment, props.NOTION_CONFIG)
...
async function convertNotionToSiteData(
  ...
  const visibleCollectionData = filterHiddenPages(collectionData, NOTION_CONFIG)
  ...
  let postCount = 0
  let allPages = visibleCollectionData.filter(post => {
  ...
  const notice = await getNotice(visibleCollectionData.find(post => post?.type === 'Notice' && post.status === 'Published'))
  ...
  const customNav = getCustomNav({ allPages: visibleCollectionData.filter(post => post?.type === 'Page' && post.status === 'Published') })
  const customMenu = getCustomMenu({ collectionData: visibleCollectionData, NOTION_CONFIG })
...
function filterHiddenPages(collectionData, NOTION_CONFIG) {
  const hiddenPageIds = getHiddenPageIds(NOTION_CONFIG)
  ...
  return collectionData.filter(
    post => !hiddenPageIds.has(normalizePageId(post?.id))
  )
}

function isHiddenPageId(id, NOTION_CONFIG) {
  const normalizedId = normalizePageId(id)
  return normalizedId
    ? getHiddenPageIds(NOTION_CONFIG).has(normalizedId)
    : false
}

function getHiddenPageIds(NOTION_CONFIG) {
  return new Set(
    toConfigList(siteConfig('NOTION_HIDDEN_PAGE_IDS', [], NOTION_CONFIG))
      .map(normalizePageId)
      .filter(Boolean)
  )
}
...
function normalizePageId(id) {
  ...
  try {
    return trimmedId.includes('-') ? trimmedId : idToUuid(trimmedId)
  } catch {
    return trimmedId
  }
}
