...
// tier 由「相关 × 质量」四象限决定（各 0–3，>=2 高 / <=1 低）：
//   高×高 → 2 进地图 / 低×低 → 0 仅档案 / 其余 → 1 雷达。
// 未评分 → 1（中性，等回溯或下次重判）。
const deriveTier = (relevance, quality, scored) => {
  if (!scored) return 1
  if (relevance >= 2 && quality >= 2) return 2
...
const toRow = paper => {
  ...
  const tier =
    paper.tier != null
      ...
      : deriveTier(relevance ?? 0, quality ?? 0, scored)
  return {
    arxiv_id: arxivId,
